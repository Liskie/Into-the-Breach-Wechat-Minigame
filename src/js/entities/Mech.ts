...
export class Mech extends Unit {
  constructor(
    ...
    public hp: number,
    public maxHp: number,
    public pilotKey: string,
    public attacksLeft: number = 1,
  ...
  static newUnit(game: Game, coords: Coords, pilotKey: string) {
    const sprite = Mech.getSprite(game, coords);
    return new Mech(game, coords, sprite,
      MechProperties.TankMaxAp, MechProperties.TankAtkRange, MechProperties.TankMaxHp, MechProperties.TankMaxHp, pilotKey);
  }
  ...
  findPossibleAttackDestinations() {
    const attackMat: Array<Array<boolean>> = [];
    for (let i = 0; i < this.game.BOARD_SIZE; i++) {
      const tempo = [];
      for (let j = 0; j < this.game.BOARD_SIZE; j++) {
        tempo.push(false);
  ...
  attackOnClick(clickCoords: Coords) {
    if (this.hp <= 0 || this.attacksLeft <= 0) {
      return;
    }
    this.attacksLeft -= 1;
    const attackMat = this.findPossibleAttackDestinations();
    let attackTargetCoords!: Coords;
    ...
    this.game.time.addEvent({
      ...
      delay: UnitProperties.ShotDelay * 10 + 300, // ms
      callbackScope: this,
      repeat: 0
    });
  }
  public shootDirectionIndex = -1;
  realAttack(tgt: Unit) {
    tgt.pushed(this.shootDirectionIndex);
    tgt.beAttacked(1);
  }

  findShotPath(desCoords: Coords) {
    const shotPath = [];
  ...
  updateHpBar() {
    this.game.mechHpBars.get(this.pilotKey).forEach((value: Rectangle, key: number) => {
      if (key <= this.hp) {
        if (this.hp == 1) {
