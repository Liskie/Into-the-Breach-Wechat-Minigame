...
export class Shot {
    constructor(game, atker, road, intention) {
        this.game = game;
        this.atker = atker;
        this.road = road;
                ...
                callback: () => {
                    ...
                    if (ii != (road.length - 1) * sf) {
                        ...
                        if (game.board[road[road.length - 1].x][road[road.length - 1].y] != null) {
                            atker.realAttack(game.board[road[road.length - 1].x][road[road.length - 1].y]);
                        }
                    }
                },
                delay: UnitProperties.ShotDelay / sf,
                callbackScope: this,
                repeat: (road.length - 1) * sf - 1
            });
        }
    }
    static getSprite(game, coords, type, intention) {
